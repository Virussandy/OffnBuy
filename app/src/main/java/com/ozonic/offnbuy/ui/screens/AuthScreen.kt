...
import com.ozonic.offnbuy.presentation.viewmodel.AuthState
    ...
    AppScaffold(
        ...
        Box(modifier = Modifier.fillMaxSize().padding(paddingValues)) {
            ...
            when (backgroundState) {
                ...
                is AuthState.AwaitingOtp -> { // Now only handles one state
                    OtpScreen(
...
fun OtpScreen(
    ...
    // This LaunchedEffect will handle the animation
    LaunchedEffect(state.autoVerifiedOtp) {
        state.autoVerifiedOtp?.let { otp ->
            // Animate the OTP filling
            otp.forEach { char ->
                otpValue += char
                delay(120) // Delay between each character for animation effect
            }
            // After animation, trigger verification automatically
            onVerifyOtp(state.verificationId, otpValue)
        }
    }
    ...
    Column(
        ...
        // Show the auto-verifying message
        if (state.autoVerifiedOtp != null) {
            Text(
